...
const OperatingSystem = ({ isOn }: OperatingSystemProps) => {
  ...
  const [isMouseInside, setIsMouseInside] = useState(false); // Track if real mouse is inside
  ...
  return (
    <div
      ...
      style={{
        cursor: isMouseInside ? `url('/assets/computer_backgrounds/mouse.svg'), auto` : 'auto', // Custom cursor on mouse enter
      }}
    >
      ...
      <div className="absolute top-[333px] left-[180px] p-[4px] h-[28px] w-[300px] bg-white/40 backdrop-blur-sm border-white/40 border rounded-md flex gap-1">
        {/* APP BAR */}
        <div className="h-[20px] w-[20px] bg-emerald-500 rounded-md" onClick={() => setOpenApp("Emerald")}></div>
        <div className="h-[20px] w-[20px] bg-cyan-500 rounded-md" onClick={() => setOpenApp("Cyan")}></div>
        <div className="h-[20px] w-[20px] bg-purple-500 rounded-md" onClick={() => setOpenApp("Purple")} onScroll={() => console.log("Scrolling at Purple")}></div>
      </div>
      {/* Only show the fake mouse cursor when the real mouse is outside */}
      {!isMouseInside && <MouseCursor />}
    </div>
  );
}
